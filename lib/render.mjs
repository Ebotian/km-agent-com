...
export function digestBlock({
  strong, weak, strongHidden = 0, weakHidden = 0, total, reader, pluginRoot, deaf = null,
}) {
  const parts = [];
  if (total > 0) {
    if (strong.length > 0) {
    ...
    // 强投递也有条数上限（posts.STRONG_MAX），被它截掉的那几条本轮没进上下文，所以同样要
    // 有一行交代：只压着不说，接收方会以为"处理完这 50 条就没事了"。
    if (strongHidden > 0) {
      parts.push(`（另有 ${strongHidden} 条点名给你的未列出：超过单轮上限，下次交互继续给你）`);
    }
  ...
  if (deaf) {
    ...
    // 那一行命令本身没错，错的是**怎么起它**：`Bash` 后台任务的默认超时是 600 秒，而
    // `--timeout 43200` 只管 watcher 自己的租约、管不到引擎那一层。少了 `disable_timeout`
    // 的武装 10 分钟后就被 SIGTERM（审计里是 `watch-stop 0 signal`），现场表现是"反复
    // timed out + 一直收不到 @"。这话必须与 rearm 行同时出现：agent 读到提示的那一刻，
    // 就是它决定怎么重新武装的那一刻。
    parts.push('  （起它的那个 `Bash` 后台任务要设 `disable_timeout: true`——否则引擎按默认 600 秒把它掐掉）');
  }
  return parts.join('\n');
}
