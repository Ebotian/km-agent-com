...
export function poll(db, { reader, strongLimit = STRONG_MAX, weakLimit = WEAK_MAX }) {
  // 两个上限都必须是不小于 **0** 的整数。负数不只是"少投几条"：`LIMIT cap + 1` 在 cap = -1
  // 时就是 `LIMIT 0`，查询返回空数组，于是下面 `strongRows.length > strongLimit`（`0 > -1`）
  // 成立、`strongRows[strongLimit]` 索引 -1 得 undefined ⇒ 读 `.seq` 抛 TypeError；负小数与
  // NaN 更早一步就被 SQLite 挡下（`datatype mismatch`），但那是数据库的抱怨，不是调用方的。
  // `cap = 0` 则**合法**（"这一轴本轮不投"）：查询取回 0+1 条、`slice(0, 0)` 投出 0 条，那条
  // 取回来的行正是溢出点，`ackUpTo` 停在游标处——正是背压该有的样子，所以判据是"≥ 0"。
  if (!Number.isInteger(strongLimit) || strongLimit < 0) {
    throw new Error(`poll 的 strongLimit 必须是不小于 0 的整数，收到 ${strongLimit}`);
  }
  if (!Number.isInteger(weakLimit) || weakLimit < 0) {
    throw new Error(`poll 的 weakLimit 必须是不小于 0 的整数，收到 ${weakLimit}`);
  }
  ...
  return {
    ...
    // 触发条件是 **strong 轴溢出**（未读点名帖 ≥ STRONG_MAX）；点名帖每分钟每主题限流 1 条
    // （见 recentDirectCount），所以罕见；重投量也有界——每轮至多 WEAK_MAX 行 triage 重复，
